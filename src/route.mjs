...
router.post("/book", authMiddleware, bookService.create);
router.get("/book/free", authMiddleware, bookService.listBookFree);
router.get("/book/booked", authMiddleware, bookService.listBooked);
router.post("/token", refreshTokenCheck, async (req, res, next) => {
  ...
  const user = await User.findOne({ refresh_token: refreshToken }).select(
    "_id name email password created_at token refresh_token"
  );
  const userData = {
    _id: user._id,
    name: user.name,
